...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        old_site = ''
        old_date = tz.localize(datetime.strptime('20000101', '%Y%m%d'))
        for f, path in recordings:
            site_code = '_'.join(f.split('_')[:2])
            date = tz.localize(datetime.strptime(f.split('_')[2], '%Y%m%d'))
            if site_code != old_site_code or (date - old_date).days > 1:
                old_site = site_code
                old_date = date
                site = Site.objects.get(code=site_code, organisation=doc)
                try:
                    Deployment.objects.get(site=site, owner=doc,
                        start=date, start_timezone='Etc/GMT+12')
                except Deployment.DoesNotExist:
                    Deployment(site=site, owner=doc,
                        start=date, start_timezone='Etc/GMT+12').save()
