...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        for ans in orm.AnalysisSet.objects.all():
            ans.datetime = ans.analysis.datetime
            ans.save()

        for identification in orm.Identification.objects.all():
            snippet = identification.snippet
            analysis = identification.analysis
            try:
                identification.analysisset = orm.AnalysisSet.objects.filter(
                    snippet=snippet,
                    analysis=analysis)[0]
                identification.save()
            except IndexError:
                "We add in an AnalysisSet record"
                ans = orm.AnalysisSet(snippet=snippet, analysis=analysis,
                    datetime=identification.datetime)
                ans.save()
                identification.analysisset = ans
                identification.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for identification in orm.Identification.objects.all():
            identification.analysis = identification.analysisset.analysis
            identification.snippet = identification.analysisset.snippet
            identification.save()
